...
path_2_snapshot_data = np.array([os.path.join(test_dir, 'distinct_1.0.fits.gz'),os.path.join(test_dir,'distinct_0.6565.fits'),os.path.join(test_dir,'distinct_0.4922.fits'),os.path.join(test_dir,'distinct_0.353.fits')])
...
edge_xoff = np.logspace(-3.6,0,51)
...
plt.figure(figsize=(10,10))
plt.scatter(xoff_rel,spinpar_rel, ls='None',label='relaxed',c='C0',marker='.')
plt.scatter(xoff_unrel,spinpar_unrel, ls='None',label='unrelaxed',c='C1',marker='.')
...
plt.xlabel(r'X$_{\rm off}$',fontsize=25)
plt.ylabel(r'$\lambda_{\rm P}$',fontsize=25)
plt.xscale('log')
plt.yscale('log')
...
plt.tight_layout()
outf=os.path.join(this_dir,'figures','Xoff_lambdaP_Rvir.png')
...
ax2.plot(log1sig1_bin,ratio_rel,linewidth=4)
ax2.plot(log1sig1_bin,ratio_unrel,linewidth=4)
ax2.plot(log1sig1_bin,ratio_comp,linewidth=4)
...
ax1_sec.plot([],[])
ax1_sec.set_xlabel(r'$\log_{10}$M [M$_{\odot}$]', fontsize=25, labelpad=15)
...
outfig = os.path.join(this_dir,'figures','MF_comparison_Rvir.png')
