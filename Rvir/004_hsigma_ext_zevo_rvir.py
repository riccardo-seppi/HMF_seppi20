...
print(type(hsigma_all.ravel()))
    #tab = Table(gsigma,names=s_bins)
    #out_table = os.path.join(this_dir,'z_%.3f'%(z_snap),'gsigma_data.fit')
...
def my_flat_priors(cube):
    ...
    params[2] = (cube[2])*0.2 - 0.05            #k2
    params[3] = (cube[3])*0.2 - 0.18      #k3
    params[4] = (cube[4])*0.2 - 0.02        #k4
    params[5] = (cube[5])*0.8 - 0.7             #k5
    params[6] = (cube[6])*0.3 - 0.1              #k6
    params[7] = (cube[7])*0.3 - 0.2     #k7
    params[8] = (cube[8])*0.9 - 0.05        #k8

    return params
#[-0.04406496 -0.16117624  0.04084333 -0.12862484  0.10813041 -0.31145948
 # 0.09023274 -0.07683871  0.61237553
...
guess_prms = [(0.0,-0.2,0.04,-0.04,0.08,-0.41,0.17,-0.2,0.26)]
...
def h_func(data,k0,k1,k2,k3,k4,k5,k6,k7,k8):
...
#    return A+np.log10(np.sqrt(2/np.pi)) + (q*opz**0.072)*np.log10(np.sqrt(a*opz**-0.14)*dc/x) - a*opz**-0.14/2/np.log(10)*dc**2/x**2 + (alpha*opz**0.066)*np.log10(y/10**(mu*opz**-0.05)/x**(e0*opz**-0.19)) - 1/np.log(10)*(y/10**(mu*opz**-0.05)/(x**(e0*opz**-0.19)))**(0.05*alpha*opz**0.066) + (gamma*opz**0.17)*np.log10(z/(0.7*10**(mu*opz**-0.05))) - 1/np.log(10)*(y/10**(mu*opz**-0.05)/x**(e1*opz**0.234))**(beta*opz**-0.43)*(z/(0.7*10**(mu*opz**-0.05)))**(delta*opz**-0.21)
    ...
    return -22.00417*(opz)**k0+np.log10(np.sqrt(2/np.pi)) + (2.28468*opz**k2)*np.log10(np.sqrt(0.88559*opz**k1)*dc/x) - 0.88559*opz**k1/2/np.log(10)*dc**2/x**2 + (5.62276*opz**k4)*np.log10(y/10**(-1.83*3.32566*opz**k3)) - 1/np.log(10)*(y/10**(-1.83*3.32566*opz**k3))**(0.05*5.62276*opz**k4) + (3.02449*opz**k6)*np.log10(z/(10**(-3.32566*opz**k3))) - 1/np.log(10)*(y/10**(-1.83*3.32566*opz**k3)/x**(-1.10492*opz**k8))**(-0.39123*opz**k5)*(z/(10**(-3.32566*opz**k3)))**(1.20909*opz**k7)
...
names = [r'$k_0$',r'$k_1$',r'$k_2$',r'$k_3$',r'$k_4$',r'$k_5$',r'$k_6$',r'$k_7$',r'$k_8$']
...
fig.tight_layout()
fig.savefig(os.path.join(this_dir,'results','zevo','corner_hsigma_zevo_rvir_ls11.png'),overwrite=True)
#############END OF ULTRANEST#########
...
plt.tight_layout()
outmodelfig_respdf = os.path.join(this_dir,'results','zevo','resid_pdf_hsigma_Rvir.png')
...
#        yerr=1/np.log(10)*g_sigma_xoff_err[j,:]/g_sigma_xoff[j,:] #get log error
    ...
    plt.grid(True)
    outfig_xo = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_sigma_xoff_mass_slices_Rvir.png')
...
#       yerr=1/np.log(10)*g_sigma_xoff_err[:,j]/g_sigma_xoff[:,j] #get log error
    ...
    plt.ylim(-5,0)
    plt.grid(True)
    outfig_mass = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_sigma_xoff_xoff_slices_Rvir.png')
...
#        yerr=1/np.log(10)*g_sigma_spin_err[j,:]/g_sigma_spin[j,:] #get log error
    ...
    plt.grid(True)
    outfig_xo = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_sigma_spin_mass_slices_Rvir.png')
    os.makedirs(os.path.dirname(outfig_xo), exist_ok=True)
    plt.savefig(outfig_xo,overwrite=True)
    plt.close()
...
#        yerr=1/np.log(10)*g_sigma_spin_err[:,j]/g_sigma_spin[:,j] #get log error
    ...
    plt.ylim(-5,0)
    plt.grid(True)
    outfig_mass = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_sigma_spin_spin_slices_Rvir.png')
    os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
    plt.savefig(outfig_mass,overwrite=True)
    plt.close()
...
#        yerr=1/np.log(10)*g_xoff_spin_err[:,j]/g_xoff_spin[:,j] #get log error
    ...
    plt.ylim(-5,0)
    plt.grid(True)
    outfig_mass = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_xoff_spin_spin_slices_Rvir.png')
    os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
    plt.savefig(outfig_mass,overwrite=True)
    plt.close()
...
#        yerr=1/np.log(10)*g_xoff_spin_err[j,:]/g_xoff_spin[j,:] #get log error
    ...
    plt.ylim(-5,0)
    plt.grid(True)
    outfig_mass = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'g_xoff_spin_xoff_slices_Rvir.png')
    os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
    plt.savefig(outfig_mass,overwrite=True)
    ...
    ax2.grid(True)
    plt.tight_layout()
    outfig_mass = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'f_xoff_integral_Rvir.png')
    os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
    plt.savefig(outfig_mass,overwrite=True)
    plt.close()
    ...
    plt.tight_layout()
    outfig = os.path.join(this_dir,'results','zevo','red_%.3f'%(z_snap),'f_integral_gsigma_Rvir.png')
