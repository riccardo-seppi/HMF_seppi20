...
path_2_snapshot_data = os.path.join(file_dir, 'distinct_1.0.fits')
...
guess_prms = [( -9, 0.9, 1.7, -1, 5.,0.45,-1.5,-2.,2.,0.4,-1,0.4,-1)]#,3.,-1,-1)] #1st June
guess_prms = [( -2, 0.9, 2.0, -3, 5.,0.3,-1.5,-3.,3.,1.1,-0.35,-2.7)] #model ok
guess_prms = [( -2, 0.9, 2.0, -3, 5.,0.05,-1.5,0.3,3.,1.1,-0.35,-2.7)] #alpha*beta and mu*nu
guess_prms = [( -2, 0.9, 2.0, -3,5.,-0.35,-1.5,3.,1.1,-2.7)] #without beta1(beta), nu
...
def h_func(data,A,a,q,mu,alpha,beta,e0,gamma,delta,e1):
    ...
    y = 10**y_   #Xoff
    z = 10**z_ #spin
    return A+np.log10(np.sqrt(2/np.pi)) + q*np.log10(np.sqrt(a)*dc/x) - a/2/np.log(10)*dc**2/x**2 + (alpha)*np.log10(y/10**mu/x**e0) - 1/np.log(10)*(y/10**mu/(x**e0))**(0.05*alpha) + gamma*np.log10(z/(0.7*10**(mu))) - 1/np.log(10)*(y/10**(mu)/x**e1)**(beta)*(z/(0.7*10**(mu)))**(delta)

#    return A+np.log10(np.sqrt(2/np.pi)) + q*np.log10(np.sqrt(a)*dc/x) - a/2/np.log(10)*dc**2/x**2 + (alpha)*np.log10(y/10**mu/x**e0) - 1/np.log(10)*(y/10**mu/(x**e0))**(beta*alpha) + gamma*np.log10(z/(nu*10**(mu))) - 1/np.log(10)*(y/10**(mu)/x**e1)**(beta1)*(z/(nu*10**(mu)))**(delta)  #model with alpha*beta, nu*mu
#    return A+np.log10(np.sqrt(2/np.pi)) + q*np.log10(np.sqrt(a)*dc/x) - a/2/np.log(10)*dc**2/x**2 + (alpha)*np.log10(y/10**mu/x**e0) - 1/np.log(10)*(y/10**mu/(x**e0))**(beta) + gamma*np.log10(z/10**nu) - 1/np.log(10)*(y/10**(mu)/x**e1)**(beta1)*(z/10**nu)**(delta) #model ok
...
fig.tight_layout()
outfig = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'corner_hsigma2.png')
...
out_table = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'hsigma_params.fit')
...
plt.tight_layout()
outmodelfig_respdf = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'resid_pdf_hsigma.png')
...
plt.ylim(-5,0)
#plt.xlim(-3.5,0.)
plt.ylabel(r'$\log_{10}g_{\lambda}(\sigma,X_{off,P})$', fontsize=20)
plt.xlabel(r'$\log_{10}X_{off,P}$', fontsize=20)
plt.grid(True)
outfig_xo = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'g_sigma_xoff_mass_slices.png')
...
#    yerr=1/np.log(10)*g_sigma_xoff_err[:,j]/g_sigma_xoff[:,j] #get log error
    ...
    plt.fill_between(s_bins,np.log10(g_sigma_xoff[:,j])-yerr,np.log10(g_sigma_xoff[:,j])+yerr,alpha=0.4,label=r'$X_{off,P} = %.3g$'%(label))
...
plt.ylim(-5,0)
plt.grid(True)
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'g_sigma_xoff_xoff_slices.png')
...
plt.ylim(-5,0)
#plt.xlim(-3.5,0.)
plt.ylabel(r'$\log_{10}g_{X_{off,P}}(\sigma,\lambda)$', fontsize=20)
plt.xlabel(r'$\log_{10}\lambda$', fontsize=20)
plt.grid(True)
outfig_xo = os.path.join(this_dir,'results',r'z_%.3f'%(z_snap),'g_sigma_spin_mass_slices.png')
os.makedirs(os.path.dirname(outfig_xo), exist_ok=True)
plt.savefig(outfig_xo,overwrite=True)
...
plt.ylim(-5,0)
plt.grid(True)
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'g_sigma_spin_spin_slices.png')
os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
plt.savefig(outfig_mass,overwrite=True)
...
plt.legend(fontsize=15)
plt.ylabel(r'$\log_{10}g_{\sigma}(X_{off,P},\lambda)$', fontsize=20)
plt.xlabel(r'$\log_{10}X_{off,P}$',fontsize=20)
plt.tick_params(labelsize=15)
plt.ylim(-5,0)
plt.grid(True)
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'g_xoff_spin_spin_slices.png')
os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
plt.savefig(outfig_mass,overwrite=True)
...
#    yerr=1/np.log(10)*g_xoff_spin_err[j,:]/g_xoff_spin[j,:] #get log error
    yerr=1/np.log(10)*err_g_xoff_spin[j,:]/g_xoff_spin[j,:]
    label = 10**xoff_bins[j]
    plt.fill_between(spin_bins,np.log10(g_xoff_spin[j,:])-yerr,np.log10(g_xoff_spin[j,:])+yerr,alpha=0.4,label=r'$X_{off,P} = %.3g$'%(label))
...
plt.ylim(-5,0)
plt.grid(True)
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'g_xoff_spin_xoff_slices.png')
os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
plt.savefig(outfig_mass,overwrite=True)
...
ax1.fill_between(xoff_bins,np.log10(f_xoff)-yerr,np.log10(f_xoff)+yerr, label=r'$f(X_{off,P})\ data$',alpha=0.8)
ax2.fill_between(xoff_bins,rat-yerr,rat+yerr,alpha=0.8)
ax2.hlines(0,min(xoff_bins),max(xoff_bins))
ax1.plot(xoff_bins,np.log10(f_xoff_model), label=r'$f(X_{off,P})\ model$',linewidth=3, color = 'red')
ax1.legend(fontsize=15)
ax1.set_ylabel(r'$\log_{10}f_{\sigma,\lambda}(X_{off,P})$', fontsize=20)
ax2.set_xlabel(r'$\log_{10}X_{off,P}$',fontsize=20)
...
ax2.grid(True)
plt.tight_layout()
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'f_xoff_integral.png')
os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
plt.savefig(outfig_mass,overwrite=True)
...
ax1.legend(fontsize=15)
ax1.set_ylabel(r'$\log_{10}f_{\sigma,X_{off,P}}(\lambda)$', fontsize=20)
ax2.set_ylabel(r'$\Delta\log_{10}f$', fontsize=20)
ax2.set_xlabel(r'$\log_{10}\lambda$',fontsize=20)
ax1.tick_params(labelsize=15)
ax2.tick_params(labelsize=15)
ax1.set_ylim(-5,0)
ax1.grid(True)
...
ax2.grid(True)
plt.tight_layout()
outfig_mass = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'f_spin_integral.png')
os.makedirs(os.path.dirname(outfig_mass), exist_ok=True)
plt.savefig(outfig_mass,overwrite=True)
...
ax1.plot(s_bins,np.log10(f_integral_model), label='seppi20',linewidth=2,c='C0')
ax1.plot(s_bins,np.log10(mf_comparat),label='comparat17',linewidth=2,c='C4',linestyle='dashed')
...
ax1.set_ylabel(r'$\log_{10}f_{X_{off,P},\lambda}(\sigma)$', fontsize=20)
...
plt.tight_layout()
outfig = os.path.join(this_dir,'results','z_%.3f'%(z_snap),'f_integral_gsigma.png')
